...
void DepthOfField::applyDOF() {
  ...
  glUniform1i(glGetUniformLocation(dof_prog, "u_depth_tex"), 1);
  
  float t = millis() * 0.01;
  glUniform1f(glGetUniformLocation(dof_prog, "u_time"), t);
  glUniform1i(glGetUniformLocation(dof_prog, "u_mode"), 0);
...
void DepthOfField::setupShaders() {
  depth_prog = createProgram(DOF_IMAGE_VS, DOF_DEPTH_FS); // debug: draw depth texture with gamma correction
