...
void SSAO::draw(const float* pm, const float* vm, const float* nm, std::vector<Particle>& particles) {
...
void SSAO::debugDraw() {
  ...
  glUniform1f(glGetUniformLocation(ssao_prog, "u_texelSize"), 1.0f/1024.0f);
  glUniform1f(glGetUniformLocation(ssao_prog, "u_occluderBias"), 0.05f);
  glUniform3f(glGetUniformLocation(ssao_prog, "u_attenuation"), 1.0f, 0.0f, 0.0f);
...
//  uniform vec2 u_attenuation; // .x linear, .y quadratic, .z exponential
...
void SSAO::setupFBO() {
  ...
  glBindTexture(GL_TEXTURE_2D, linear_depth_tex);
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, fbo_w, fbo_h, 0, GL_RGBA, GL_FLOAT, NULL);
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
